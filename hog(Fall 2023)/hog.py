...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    sum, flag = 0, 0  #flag用于记录是否存在sowsad
    while num_rolls > 0:
        curr = dice()
        if curr == 1:
            flag = 1
...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    digit_one = opponent_score % 10
    digit_ten = int((opponent_score - digit_one) * 0.1 % 10)
    return 2 * abs(digit_ten - digit_one) + 1
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        while opponent_score < goal-1:
            if strategy(score, opponent_score) != strategy(score, opponent_score + 1):
                return False
            opponent_score += 1
        score += 1
        opponent_score = 0 #注意将对手分数清零，否则无法判断己方分数变化下对方分数变化的情况（一直为98）
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def average_of_original_function(*args):
        sum, i = 0, total_samples
        while i > 0:
            sum += original_function(*args)
            i -= 1
        average = sum / total_samples
        return average
    return average_of_original_function
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    while i <= 10:
        curr_average = make_averaged(roll_dice, total_samples)(i, dice)
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    return 6  # Remove this line once implemented. 牛哇，自己写策略
    # END PROBLEM 12
