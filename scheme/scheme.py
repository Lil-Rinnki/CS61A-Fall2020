...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions != nil:
            ...
            expressions = expressions.rest          #此处expressions是一个Pair类
        return ans
    # END PROBLEM 7
...
class Frame(object):
    ...
    def define(self, symbol, value):
        ...
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        self.bindings[symbol] = value   #注意self.bindings是一个dict类型，相当于在字典中添加元素
        # END PROBLEM 2
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        env.define(target, scheme_eval(expressions.rest.first, env)) #由于expressions.rest中包含nil，故选expressions.rest.first
        return target
        # END PROBLEM 5
        ...
        LambdaE = Pair(formals, body)
        env.define(name, do_lambda_form(LambdaE, env))
        return name
        # END PROBLEM 9
    else:
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    return expressions.first #不需要nil
    # END PROBLEM 6
...
def do_lambda_form(expressions, env):
    ...
    """
    # 特别需要注意上例中LambdaProcedure对应得到的formals和body的格式，(x) -> Pair('x', nil), (+ x 2)) -> Pair(Pair('+', Pair('x', Pair(2, nil))), nil)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions != nil:
            ans = scheme_eval(expressions.first, env)
            if is_true_primitive(ans):          #注意此处引入中间变量ans，不能一直使用scheme_eval函数否则会执行两次操作
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        values = Pair(scheme_eval(bindings.first.rest.first,env), values)   #特别注意如果是bindings.first.rest则返回一个带nil的Pair格式值，需要通过bindings.first.rest.first取值
