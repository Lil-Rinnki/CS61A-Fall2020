...
def choose(paragraphs, select, k):
    ...
    the empty string.
    """
    # BEGIN PROBLEM 1 ——使用 for element in s 语句对列表s中的元素依次进行判断，注意第k段要考虑第0个元素
    "*** YOUR CODE HERE ***"
...
def about(topic):
    ...
    def select(paragraph):
        changed_para = split(remove_punctuation(lower(paragraph)))
        for element in changed_para:
            for word in topic:
                if word == element:
                    return True
        return False
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    m = len(typed_words)     #输入列表的长度记为m
    n = len(reference_words) #参考列表的长度记为n
    def real_A(i):
        if i == 0:
            return 0
        else:
            if typed_words[i-1] == reference_words[i-1]:
                return real_A(i-1) + 1
            else:
                return real_A(i-1)
    if m == 0:
        ...
        return round(100 * real_A(m) / m, 2)
    else:
        return round(100 * real_A(n) / m, 2)
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4 记输入字符数为n，则计算策略为(n/5)/(elapsed/60)=12*n/elapsed
    "*** YOUR CODE HERE ***"
    n = len(typed)
    return round(12 * n / elapsed, 2)
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if min(diff_list) > limit:
        ...
        return min(valid_words, key = lambda x: diff_function(user_word, x, limit))   #使用key寻找目标函数的min值
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    their lengths.
    """
    # BEGIN PROBLEM 6 —— 使用列表的递归！！！这写法也忒绝了
    if limit < 0:
        return 0
    elif start == '' or goal == '':
...
def pawssible_patches(start, goal, limit):
    ...
    # BEGIN PROBLEM 7 —— 上一题夸早了，这也太顶级了。重点思维是不要管到底选择1)加、2)减、3)替代哪一种修改方法，优先选择改变次数最少的
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    def time_cal(time_stamp):        #针对内层运算先定一个计算函数方便使用
        oneplayer_time = []
        for index in range(len(time_stamp) - 1):
            oneplayer_time += [time_stamp[index + 1] - time_stamp[index]]
        return oneplayer_time
