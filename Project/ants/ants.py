...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        "*** YOUR CODE HERE ***"
        if self.exit != None:
           exit.entrance = self     #self指当前创建的place
        # END Problem 2
...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem Optional 2  此处的逻辑需理清！
            old_ant = place.ant
            new_ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while curr_place != beehive:        #注意beehive的使用
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        bees_nearby = self.place.bees[:]
        Ant.reduce_armor(self, amount)                              #注意在使用上级Method使，object对象要确定好
        for nearbees in bees_nearby:
            if self.armor > 0:
                Insect.reduce_armor(nearbees, amount)
            else:
                Insect.reduce_armor(nearbees, amount + self.damage)
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):         #此处的bee如何使用？
        # BEGIN Problem 6
        "*** YOUR CODE HERE ***"
        bee.reduce_armor(bee.armor) #为类，使用Method更改
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        ScubaThrower.__init__(self, armor)          #使用parent类ScubaThrower
        QueenAnt.is_truequeen += 1
        self.falseQueen = QueenAnt.is_truequeen
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.falseQueen:
            ...
            ThrowerAnt.action(self, gamestate)      #自身也投掷攻击
            curr_place = self.place.exit
            while curr_place != None:
                if curr_place.ant != None and curr_place.ant.buff == 0:
                    curr_place.ant.damage *= 2
                    curr_place.ant.buff = 1
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.falseQueen ==0 :
            bees_win()
    
    def remove_from(self, place):                   #覆写remove_from函数，但题目好像要求改Ant类中的函数
        if self.falseQueen:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant != None and self.place.ant.blocks_path == True:
            return True
        else:
            return False
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        for nearbees in bees_nearby:
            if self.armor > 0:
                Insect.reduce_armor(nearbees, self.damage)
        # END Problem Optional 1
...
class ContainerAnt(Ant):
    ...
    def contain_ant(self, ant):
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        if self.can_contain(ant):       #判定能装下再装
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        # BEGIN Problem Optional 3
        "*** YOUR CODE HERE ***"                #既有tank内的ant攻击，也有tank本身对于该格内的bee的攻击v
        if self.contained_ant:
            self.contained_ant.action(gamestate)        
        bees_nearby = self.place.bees[:]
        for nearbees in bees_nearby:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while curr_place != beehive:        
            if curr_place.ant and self != curr_place.ant:   #如果有昆虫，则添加到字典——怎么添加到字典呢？
